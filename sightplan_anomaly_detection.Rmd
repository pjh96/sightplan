...
total_time <-  as.numeric(task_report_thru - base_day)
total_months <- total_time/30

#read in all data
data_path <- "C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/task_logs_new/"
...
#property mapping - taken from microsoft teams
#will no longer be necessary after integration ID is added 

mapping <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/sightplan_location_names.xlsx", sheet = "Sheet1") %>% clean_names() %>% select(community_name:yardi)
...
con3 <- dbConnect(odbc::odbc(), Driver = "SQL Server", Server = "essexBI.essexpropertytrust.com", 
    database = "Dimensions", Trusted_Connection = "True", pwd = "Newhire3915!", 
    UID = "phummelt", Port = 1433)
...
dimprop <- read.csv("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/dimprop.csv", colClasses = "character")

#join mapping with DW info
...
write.csv(to_export, "C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/full_maintenance_export.csv")
```


```{r maintenace tasks only}
#MAINTENANCE TASKS FOR ANALYSIS
#filter for only maintenance tasks after 2020-04-01 (after rollout to all properties)
maintenance <- tasks %>% filter(workspace=="Maintenance" & status == "Closed" & created_date >= '2020-04-01')
maintenance$time_to_complete <- as.numeric(difftime(maintenance$closed_date, maintenance$created_date, units = "days"))

#More precise time to complete?
#Need to confirm with Stephanie P. - 2020-06-03
#----
maintenance$start_time <- "1899-12-31 00:00:00"
maintenance$start_time = ymd_hms(maintenance$start_time)
maintenance$end_time = ymd_hms(maintenance$time_opened)
maintenance$time_to_complete2 = difftime(maintenance$end_time,maintenance$start_time,units = "days")
#----
...
headcount <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/maintenance_head_count.xlsx", sheet = "Sheet2", col_types = "text") %>% 
  clean_names() %>% 
...
#Open positions
ops_open <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/Weekly Open Jobs Vacancy Report April 2020.xlsx", sheet = "Ops - Open Jobs Report") %>% 
  clean_names() %>%
  mutate(row_num = row_number()) %>%
  filter(row_num != 1) %>%
  filter(row_num != 2) %>%
...
ops_freeze <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/Weekly Open Jobs Vacancy Report April 2020.xlsx", sheet = "Ops - Positions No Req LUIn") %>% 
  clean_names() %>%
  mutate(row_num = row_number()) %>%
  row_to_names(2) %>%
  rename(row_num = '2') %>%
...
#budgeted positions
ss <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/FTE_BudFY20.xlsx", sheet = "FTE - Same Store") %>%
  row_to_names(1) %>%
  clean_names() %>%
...
jv <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/FTE_BudFY20.xlsx", sheet = "FTE - Total JV") %>%
  row_to_names(1) %>%
  clean_names() %>%
  filter(grepl("\\(",property)) %>%
  separate(property, into = c("property","property_code"), sep = "\\(", ) %>%
...
nss <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/FTE_BudFY20.xlsx", sheet = "FTE - Non Same Store") %>%
  row_to_names(1) %>%
  clean_names() %>%
  filter(grepl("\\(",property)) %>%
  separate(property, into = c("property","property_code"), sep = "\\(", ) %>%
...
median_prop_size <- median(dimprop$reporting_unit_count, na.rm = T)
...
summary <- maintenance %>% 
  ...
  summarise(count = n(),
            ...
            count_per_month_per_hundred_units = ((count/2)/units)*median_prop_size) %>% #relace with median property size  #April 1 - May 24 
...
summary <- summary %>% left_join(one_star, by = c("yardi" = "client_location_id"))
summary$n <- ifelse(is.na(summary$n),0,summary$n)
...
# summary %>%
#   dplyr::arrange(submarket, residuals) %>%
#   filter(region.x == "Seattle Metro") %>%
#   select(location, submarket, residuals, units) %>% View()
...
plot <- ggplot(summary2,aes(x = expected_total, y = actual_total, col = `Operational Maintenance Vacancy`, group = 1, text = paste("Property: ", location, "<br>Region: ", region,"<br>Submarket: ",submarket, "<br>Units: ",reporting_unit_count))) +
  geom_point() +
  #geom_abline(intercept = 0, slope = 1, color = "red", size = 0.5, linetype = "longdash") + 
  ...
  #scale_color_manual(values = c("#FE9666","#8AD4EB","#01B8AA"))+
  #theme_light() +
  theme(legend.title = element_blank()) 
...
ggplotly(plot, tooltip = "text") %>%
  ...
  layout(title = list(text = paste0('Expected vs Actual Task Days - Non-Urgent',
                                    ...
                                    '<br>',
                                    'Slope for > 10% OMV - 2.13 | Slope for < 10% OMV - 1.26',
                                    '</sup>')))
```
...
to_map <- left_join(summary2,dimprop, by = c("yardi"="property_code"))
...
leaflet(to_map) %>%
  ...
  addCircleMarkers(lng = to_map$longitude, lat = to_map$latitude, radius = 6,weight = 1, fillOpacity = 0.8, fillColor = ~pal(pct_of_exp), label = to_map$location, popup=paste("Property:", to_map$location, "<br>",
                         "Actual/Expected:", round(to_map$pct_of_exp,2), "<br>",
                         "Units:", to_map$reporting_unit_count), color = "black") %>%
  addLegend(pal = pal,
            #colors = c("#b60a1c","#ff684c","#8ace7e","#309143"),
            values = to_map$pct_of_exp,
...
# ggplot(summary, aes(x = `Maintenance Workers per 100 Units`, y = actual_total)) +
#   geom_point() +
#   xlab("Maintenance Workers per 100 Units") +
#   ylab("Actual Task Days") +
...
dimprop <- dbGetQuery(con3, 'SELECT *  
                 FROM Dimensions.dim.Property ') %>% clean_names() 
...
#MEDIAN - how to work around MAD of 0 for many subcategories
#----
summary <- maintenance %>% 
  filter(source == "Answering Service" | source == "Resident App (iOS)" |
           source == "Resident App (Android)" | source == "Resident Portal") %>%
...
summary <- maintenance %>% 
  ...
  summarise(count = n(),
            ...
            property_avg_time = median(time_to_complete), #CHANGE TO MEDIAN OR MEAN
            portfolio_avg_time = mean(portfolio_avg_time), #KEEP AS MEAN, THIS IS THE MEDIAN MEASURE BUT MEAN OF 1 NUMBER IS ITESELF
            count_per_month_per_hundred_units = ((count/1.75)/units)*median_prop_size) %>% #relace with median property size  #April 1 - May 24 
  mutate(expected_task_days = portfolio_avg_time * count_per_month_per_hundred_units,
         actual_task_days = property_avg_time * count_per_month_per_hundred_units) %>%
  filter(units > 2) %>%
  ungroup() %>%
...
hours_worked <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/maintenance_head_count.xlsx", sheet = "Sheet3", col_types = "text") %>%
  clean_names() %>% 
  rename(employee_id = row_labels) %>%
  filter(employee_id != "Grand Total") %>%
...
```{r replication of maintenance KPI, echo = F, warning = F}
...
```{r expected versus man days}
#use resident reported only - non resident reported not on the same time zone
summary <- maintenance %>% 
  group_by(location) %>%
  ...
  #filter(urgency == "Not Urgent") %>%
  group_by(category,subcategory) %>%
  mutate(portfolio_avg_time = mean(time_to_complete)) %>% #CHANGE TO MEDIAN OR MEAN
  ungroup() %>%
  group_by(location,category ,subcategory) %>% 
  summarise(resident_count = n(),
            ...
            count_per_week = (resident_count/total_time)*7) %>% #April 1 - May 31
...
summary <- summary %>% 
  ...
  mutate(man_days_per_week = (count*7),
         man_hours_per_week_per_unit = (avg_hours*7*count)/reporting_unit_count,
...
plot <- summary %>%
  ...
  geom_point() +
  geom_smooth(method = "lm") + 
  labs(x = "Expected Task Days per Week", y = "Available Maintenance Days per Week", title = "Availalble Maintenance Work Days per Week and Avg. Expected Task Days",subtitle = "Expected total based on resident reported tasks only") +
...
#change colored as needed.
p <- ggplot(subset, aes(x = actual_over_expected, y = man_days_per_week, group = 1, label = location, col  = `Workload (Total Tasks/Units)`, size = reporting_unit_count, text = paste("Property: ",location,'<br>Submarket: ', submarket, "<br>Units: ", reporting_unit_count))) +
  geom_point() +
  geom_smooth(method = "lm", show.legend = F, se = F) +
  labs(x = "Actual/Expected Avg. Task Days", y = "Available Maintenance Days per Week", title = "Available Maintenance Days per Week vs. Resident Reported Task Completion Efficiency",caption = "", subtitle = region_choice) +
...
ggplotly(p, tooltip = "text") %>%
    layout(title = list(text = paste0('Available Maintenance Days per Week vs. Resident Reported Task Completion Efficiency',
                                    '<br>',
...
```{r expected v actual task days urgent, echo = F, warning = F, message = F, fig.height=10, fig.width=10, eval=F}
#include open positions?
headcount <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/maintenance_head_count.xlsx", sheet = "Sheet2", col_types = "text") %>% 
  clean_names() %>% 
  rename(employee_id = row_labels) %>%
...
lu_open <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/Weekly Open Jobs Vacancy Report April 2020.xlsx", sheet = "Lease Up Only - Open Jobs") %>% 
  clean_names() %>%
  mutate(row_num = row_number()) %>%
  filter(row_num != 1) %>%
  filter(row_num != 2) %>%
...
lu_freeze <- read_excel("C:/Users/phummelt/OneDrive - Essex Property Trust, Inc/Local_R_Material/Data/SightPlan_Data/Weekly Open Jobs Vacancy Report April 2020.xlsx", sheet = "Lease Up - Positions No Req") %>% 
  clean_names() %>%
  mutate(row_num = row_number()) %>%
  row_to_names(2) %>%
  rename(row_num = '2') %>%
...
open <- rbind(ops_open,ops_freeze)
open <- rbind(open, lu_open)
open <- rbind(open, lu_freeze)
...
summary <- maintenance %>% 
  ...
  filter(subcategory != "Voicemail") %>%
     filter(urgency == "Urgent") %>%
  group_by(category,subcategory) %>%
  mutate(portfolio_avg_time = mean(time_to_complete)) %>%
  ungroup() %>%
  group_by(location,category,subcategory) %>% 
  summarise(count = n(),
            units = mean(units),
            property_avg_time = mean(time_to_complete),
            portfolio_avg_time = mean(portfolio_avg_time),
            count_per_month_per_hundred_units = ((count/1.75)/units)*100) %>% #April 1 - May 17 
  mutate(expected_task_days = portfolio_avg_time * count_per_month_per_hundred_units,
         actual_task_days = property_avg_time * count_per_month_per_hundred_units) %>%
  filter(units > 100) %>%
  ungroup() %>%
...
summary <- summary %>% left_join(headcount_summary, by = "yardi")
summary <- summary %>% left_join(ov, by=c("yardi"="property_code"))
summary$maint_per_100 <- (summary$count.x/summary$units) * 100
summary <- summary %>% filter(!is.na(omv))
...
#high_actual <- filter(summary, actual_total >= 250)
#low_actual <- filter(summary, expected_total > 150 & expected_total<200 & actual_total < 200)
#low_actual2 <- filter(summary, expected_total > 250 & actual_total < 200)


outliers <- summary %>% filter(actual_total >= 150)
...
outliers <- summary %>% filter(expected_total > 115)
...
outliers <- summary %>% filter(location == "Park Viridian")
...
outliers <- summary %>% filter(location == "Windsor Ridge" | location == "The Blake LA" | location == "The Dylan" | location == "Park Highland" | location == "BellCentre")
...
outliers <- summary %>% filter(location == "Bella Villagio")
...
plot <- ggplot(summary2,aes(x = expected_total, y = actual_total, col = `Operational Maintenance Vacancy`, group = 1, text = paste("Region: ", region,"<br>Property: ", location, "<br>Residuals: ", residuals, "<br>Units: ",units))) +
  geom_point() +
  #geom_abline(intercept = 0, slope = 1, color = "red", size = 0.5, linetype = "longdash") + 
  geom_smooth(method = lm, se = F, color = "red", linetype = "longdash",size = 0.5) +
  labs(title = "Expected vs Actual Task Days - Urgent", subtitle = "per Month per 100 Units")+
  xlab("Expected Total Task Days") +
  ylab("Actual Total Task Days") +
  #scale_color_manual(values = c("#FE9666","#8AD4EB","#01B8AA"))+
  #theme_light() +
