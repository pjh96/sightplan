...
maintenance <- tasks %>% filter(workspace=="Maintenance" & status == "Closed" & (created_date >= '2020-04-01' & created_date <= '2020-05-20'))
...
```{r expected versus availalbe maintenance days}
...
summary <- maintenance %>% 
  ...
  summarise(resident_count = n(),
            ...
            count_per_week = ((resident_count/total_time)*7)/units) %>% #April 1 - May 31-95th percentile days to complete
...
region_choice <- "Northern California"
...
p <- ggplot(subset, aes(x = actual_over_expected, y = man_hours_per_week_per_unit , group = 1, label = location, col  = `Workload (Total Tasks/Units)`, size = reporting_unit_count, text = paste("Property: ",location,'<br>Submarket: ', submarket, "<br>Units: ", reporting_unit_count))) +
  ...
  labs(x = "Actual/Expected Avg. Task Days(Inefficiency)", y = "Available Maintenance Hours per Week per 100 Units", title = "Available Maintenance Time and Actual vs Expected Time to Complete Tasks",caption = "", subtitle = region_choice) +
