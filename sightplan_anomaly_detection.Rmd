...
ops_open <- read_excel("Weekly Open Jobs Vacancy Report April 2020.xlsx", sheet = "Ops - Open Jobs Report") %>% 
  ...
  filter(grepl("Maintenance", job_posting_title)|grepl("Porter", job_posting_title) | grepl("Painter", job_posting_title) | grepl("Landscap",job_posting_title) | grepl("Enginee",job_posting_title)) %>%
...
ops_freeze <- read_excel("Weekly Open Jobs Vacancy Report April 2020.xlsx", sheet = "Ops - Positions No Req LUIn") %>% 
  ...
  filter(grepl("Maintenance", open_position_title)|grepl("Porter", open_position_title) | grepl("Painter", open_position_title) | grepl("Landscap",open_position_title) | grepl("Enginee",open_position_title)) %>%
...
ss <- read_excel("FTE_BudFY20.xlsx", sheet = "FTE - Same Store") %>%
  ...
  select(property, property_code, maintenance) %>% 
  gather(3, key = "key",value = "value") %>%
...
summary <- maintenance %>% 
  ...
  summarise(expected_total = sum(expected_task_days),
            actual_total = sum(actual_task_days),
            efficiency = expected_total/actual_total,
...
summary <- summary %>% 
  ...
  mutate(man_days_per_week = (count*7),
         ...
         expected_over_actual = expected_total/actual_total)
...
p <- ggplot(subset, aes(x = actual_over_expected, y = man_hours_per_week_per_unit , group = 1, label = location, col  = `Workload (Total Tasks/Units)`, size = reporting_unit_count, text = paste("Property: ",location,'<br>Submarket: ', submarket, "<br>Units: ", reporting_unit_count))) +
  ...
  labs(x = "Actual / Expected Mean Task Days (Inefficiency)", y = "Available Maintenance Hours per Week per 100 Units", title = "Available Maintenance Time and Actual vs Expected Time to Complete Tasks",caption = "Efficiency measure based on resident reported tasks only", subtitle = region_choice) +
...
```{r looking at full time employees}
full_time <- hours_worked %>% filter(date <= "2020-05-20")#period of interest

#average hours per week (sum/days worked * 5)
full_time %>%
  group_by(employee_id) %>%
  mutate(days_worked = n()) %>% 
  summarise(sum_hours = sum(as.numeric(hours_worked)),
            days_worked = as.numeric(mean(days_worked)),
            weeks_worked = days_worked/5,
            first_day = min(date),
            last_day = max(date)) %>%
  ungroup() %>%
  mutate(avg_hours_per_week = (sum_hours/days_worked)*5) %>% View()

#sum of hours each week
week_end <- as.Date(c("2020-04-07","2020-04-14","2020-04-21","2020-04-28","2020-05-05","2020-05-12","2020-05-19"))
full_time %>%
  group_by(employee_id) %>%
  mutate(days_worked = n()) %>% 
  complete(date = seq.Date(as.Date("2020-04-01"),as.Date("2020-05-20"),by="day")) %>%
  mutate(hours_worked = ifelse(is.na(hours_worked),0,hours_worked),
         hours_worked = as.numeric(hours_worked),
         hours_in_the_week = roll_sum(hours_worked,7,align="right",fill = NA)) %>% 
  filter(date %in% week_end) %>% 
  arrange(employee_id) %>% View()
```
